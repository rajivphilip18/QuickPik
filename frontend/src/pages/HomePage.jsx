...
export default function HomePage({ navigate }) {
  ...
  async function handleJoin() {
    ...
    if (!res.ok) return setJoinError('Poll not found. Check the ID and try again.')
    navigate('vote', id)
  }
      ...
      <Row className="g-4">
        ...
        <Col md={6}>
          <Card className="p-4 h-100">
            ...
            <Form.Control className="mb-2" value={joinId}
                    ...
                    <ListGroup.Item key={p._id}>
                      <Row className="align-items-center">
                        <Col>
                          ...
                          <Badge bg={p.status === 'open' ? 'success' : 'secondary'} className="ms-2">
                            {p.status}
                          </Badge>
                          ...
                          <Row className="g-2">
                            <Col xs="auto">
                              <Button size="sm" variant="outline-primary"
                                onClick={() => navigate('manage', p._id)}>View</Button>
                            </Col>
                            <Col xs="auto">
                              <Button size="sm" variant="outline-danger"
                                onClick={() => deletePoll(p._id)}>Delete</Button>
                            </Col>
                          </Row>
                        </Col>
                      </Row>
